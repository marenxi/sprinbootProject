...
layui.define(['element', 'form', ], function (exports)
{
    ...
    var transferTool = {
        ...
        transferToolSelectedData: "",
        transferToolUnSelectedData: "",
        /* 入口函数 */
        init: function (data)
        {
            var that = this;
        ...
        renderData: function (R)
        {
            ...
            if (_cascade === false)
            {
                _searchHtml = [
                    ...
                    '<input class="layui-input search_condition" style="width:220px;float:right;" placeholder="关键字搜索">',
            ...
            if (data !== undefined)
            {
                ...
                {
                    //设置默认值(向右侧插入元素)
                    if (_values.indexOf(data[i].code.toString()) == -1)
                    {
                        ...
                        //设置禁用
                        if (_disableds.indexOf(data[i].code.toString()) != -1)
                        {
            ...
            var outHtml =
                $(config.elem).html([
                    '<div class="transfer-content" style="width:640px;height: 400px;position: relative">',
                    ...
                    '<a title="右移" lay-name="' + _name + '" class="layui-btn layui-btn-normal layui-btn-sm layui-btn-disabled"><i class="layui-icon">&#xe65b;</i></a>',
                    '</div>',
                    '<div class="transfer-btn  transfer-to-left">',
                    '<a title="左移" lay-name="' + _name + '"  class="layui-btn layui-btn-normal layui-btn-sm layui-btn-disabled"><i class="layui-icon">&#xe65a;</i></a>',
                    '</div>',
        ...
        {
            return transferTool.transferToolSelectedData;
        },
        getUnSelectedData: function ()
        {
            return transferTool.transferToolUnSelectedData;
        }
    }
    ...
    function updataValue()
    {
        ...
        transferTool.transferToolSelectedData = "";
        transferTool.transferToolUnSelectedData = "";
        ...
        else
        {
            for (var i = 0; i < selList.length; i++)
            {
                ...
                transferTool.transferToolSelectedData += _value + ",";
            }
        }
        ...
        else
        {
            for (var i = 0; i < unSelList.length; i++)
            {
                ...
                transferTool.transferToolUnSelectedData += _value + ",";
            }
        }
    }
